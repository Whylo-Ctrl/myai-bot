...
PERSONALITY_MODES = {
    "assistant": """
You are MyAI, a helpful Discord assistant.
Be casual, clear, and human.
...
""",
    "streamer": """
You are MyAI, a funny, cocky, aggressive Discord bot with streamer energy.
Talk like a real person in a gaming server.
Be confident, talk your shit a little, and keep it entertaining.
Roast people lightly when it fits.
Be bold, funny, and a little disrespectful in a joking way.

Important:
- DO NOT type in all caps unless you're emphasizing one or two words
- DO NOT do long cringe speeches
- DO NOT sound like a corporate assistant
- Keep replies natural, sharp, and conversational
- Use normal sentence case most of the time
- Be more "bro what are you talking about 😭" and less "WELCOME BACK MASHED POTATO LOVERS"
""",
    "savage": """
You are MyAI, a cocky, funny, aggressive Discord bot.
Be sarcastic, sharp, and entertaining.
Roast people when it fits, but keep it playful.
Do not be hateful or extreme.
Do not type in all caps all the time.
...
- If something is current/live and you aren't fully sure, say so briefly instead of making it up
- Never confidently invent fake current facts
- Use normal capitalization
- Avoid walls of text unless the user clearly wants detail
"""
...
def clean_reply(reply: str) -> str:
    if not reply:
        return "i got nothing right now 😭"
    ...
    # stop constant all-caps yelling
    ...
    if any(word in content_lower for word in ["nickname", "rename", "change name"]):
        ...
        if target_member and new_nick:
            try:
                ...
                await message.channel.send("couldn't change that nickname.")
            return
    ...
    if "channel" in content_lower and any(word in content_lower for word in ["create", "make"]):
        ...
        if channel_name:
            ...
            try:
                ...
                await message.channel.send("couldn't create that channel.")
            return
    ...
    if "role" in content_lower and any(word in content_lower for word in ["create", "make"]):
        ...
        if role_name:
            ...
            try:
                ...
                await message.channel.send("couldn't create that role.")
            return
    ...
    if "role" in content_lower and any(word in content_lower for word in ["give", "add"]):
        if message.mentions:
            ...
            if role_name:
                ...
                if not role:
                    await message.channel.send(f"couldn't find a role named **{role_name}**")
                    return
                try:
                    ...
                    await message.channel.send("couldn't give that role.")
                return
